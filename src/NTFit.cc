...
NTFit::NTFit(int iType, TLorentzVector lorentz1, TLorentzVector lorentz2, double met, double phi, const TMatrixD covMet, std::string iLikelihoodFile, NTFit::Options iOption, NTFit::CalcOptions iCalcOption, int iNEvents, bool iCorrectMet) : fcovMet(2,2)
{ 
  // input variables
  fVec0 = lorentz1; fVec1 = lorentz2;
  fMet = met; fMetPhi = phi;
  fcovMet(0,0) = covMet(0,0);
  fcovMet(1,0) = covMet(1,0);
  fcovMet(0,1) = covMet(0,1);
  fcovMet(1,1) = covMet(1,1);
...
TLorentzVector NTFit::NeutrinoToys(int iNProng,int iNJet,double iD01,double iD02) 
{ 
  // define inputs 
  double iMet = fMet; double iMetPhi = fMetPhi; 
  double iPt1 = fVec0.Pt(); double iPhi1 = fVec0.Phi(); double iEta1 = fVec0.Eta(); double iM1=fVec0.M();
  double iPt2 = fVec1.Pt(); double iPhi2 = fVec1.Phi(); double iEta2 = fVec1.Eta(); double iM2=fVec1.M();
  double iMCov00 = fcovMet(0,0);double iMCov10 = fcovMet(1,0);double iMCov01 = fcovMet(0,1);double iMCov11 = fcovMet(1,1);
  // clear up stuff
